...
function* createSongAsync(action: {payload: Pick<SongType, 'album' | 'artist' | 'genre' | 'title'>}) {
  try {
    ...
    yield put(songAction.songError(error.message));
  }
}
...
function* updateSongAsync(action: {payload: SongType}) {
  try {
...
export function* removeSongStart() {
  yield takeLatest(songAction.deleteSongSuccess, removeSongAsync);
}
